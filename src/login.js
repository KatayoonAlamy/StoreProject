...
export const login = function () {
  root.innerHTML = `
    <figure class="animated-up"><img src="./external/imges/prev icon.png" alt=""></figure>
      <form autocomplete="off" class="animated-up flex flex-col gap-4 items-center justify-center mt-28">
        ...
        <p class="font-semibold text-4xl mt-12">Login to your Account</p>
        ...
        <a href="/signup" data-navigo><p  class="text-blue-600">Dont't have an account?</p></a>
        <button class="mt-72 w-96 p-2 rounded-3xl bg-gray-400 text-white" id="signInButton">Sign in</button>
      </form>
    `;
  ...
  signInButton.addEventListener("click", async (e) => {
    ...
    try {
      const response = await axios.post(`${newBaseUrl}/login`, credit);
  ...
  const checkInputs = () => {
    if (emailInput.value.trim() !== "" && passwordInput.value.trim() !== "") {
      signInButton.classList.remove("bg-gray-400");
      signInButton.classList.add("bg-[#212529]");
    } else {
      signInButton.classList.remove("bg-[#212529]");
      signInButton.classList.add("bg-gray-400");
    }
  };
